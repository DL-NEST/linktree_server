...
func InitMqtt() *MqState {
	// 创建mqtt客户端
	MqttClient = &MqState{
		MqttClient: CreateMqttClient(),
		LinkList:   make(map[string]Sub),
	}
	// 在协程里面监听主题
...
// Subscribe 订阅主题
func Subscribe(mqState *MqState, topic string, qos int) {
	mqState.LinkList[topic] = Sub{
		name: topic,
		qos:  qos,
	}
	mqState.MqttClient.Subscribe(topic, byte(qos), MessageHandler)
}
